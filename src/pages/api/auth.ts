...
export async function POST(req: NextApiRequest, res: NextApiResponse) {
  ...
  const signerAddress = await ethers.utils.verifyMessage(
    buildSignMessage(payload.address, payload.time),
    payload.signature
  );
  ...
  if (user.lastSeen.getTime() >= getCurrentUTC().getTime()) {
    return res.status(406).json({ errors: [`LoginFailed`] });
  }
  ...
  return res
    ...
    .json({
      user,
      token,
      key: process.env.accessTokenName,
      time: getCurrentUTC(),
    });
}
...
export async function DELETE(req: NextApiRequest, res: NextApiResponse) {
  ...
  return res.status(202).json({ time: getCurrentUTC() });
}
...
export async function PUT(
  ...
  return res.status(200).json({ user: _user, time: getCurrentUTC() });
}
